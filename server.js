...
// Import Advanced Plagiarism Engine
const AdvancedPlagiarismEngine = require('./advanced-plagiarism-engine.js');

const app = express();
const PORT = 3000;

// Initialize Advanced Plagiarism Engine
const advancedEngine = new AdvancedPlagiarismEngine();
        ...
        switch (fileExtension) {
            ...
            case '.pdf':
                ...
                try {
                    ...
                    return await extractPDFAlternative(filePath, originalName);
                }
...
// Alternative PDF extraction method for problematic PDFs
async function extractPDFAlternative(filePath, originalName) {
    ...
    try {
        ...
        if (pdfData.text && pdfData.text.trim().length > 0) {
            console.log(`✅ Alternative method 1 successful: ${pdfData.text.length} characters`);
            return pdfData.text;
        }
        
        // Method 2: Try with different settings
        const pdfData2 = await pdfParse(pdfBuffer, {
            normalizeWhitespace: true,
            disableCombineTextItems: true
        });
        
        if (pdfData2.text && pdfData2.text.trim().length > 0) {
            console.log(`✅ Alternative method 2 successful: ${pdfData2.text.length} characters`);
            return pdfData2.text;
        }
        ...
        // Last resort: Return basic error message with file info
        return `[PDF_EXTRACTION_FAILED] File: ${originalName}. 
        ...
        Please try converting to TXT or DOCX format for better results.`;
    }
}
...
function validateAndSanitizeText(text, filename = '') {
    ...
    // Check if this is a PDF extraction failure message
    if (trimmedText.includes('[PDF_EXTRACTION_FAILED]')) {
        throw new Error(`PDF extraction failed for ${filename}. Please try converting to TXT or DOCX format.`);
    }
...
function preprocessText(text) {
    // First, remove obvious formatting elements that shouldn't be considered for plagiarism
    let cleanedText = text
        .replace(/^\s*[\d\.\)\(\-\*\+\•]\s*/gm, '') // Remove list numbering at start of lines
        .replace(/^\s*[a-zA-Z]\.\s*/gm, '') // Remove letter numbering (a. b. c.)
        .replace(/^\s*[IVX]+\.\s*/gm, '') // Remove Roman numeral numbering
...
function createNGrams(text, n = 3) {
    ...
    // Helper function to check if an n-gram is meaningful
    function isMeaningfulNGram(ngramWords) {
        // Skip if too many numbers
        const numberCount = ngramWords.filter(word => /^\d+$/.test(word)).length;
        if (numberCount > ngramWords.length / 2) return false;
        
        // Skip if too many very short words
        const shortWordCount = ngramWords.filter(word => word.length <= 2).length;
        if (shortWordCount > ngramWords.length / 2) return false;
        
        // Skip if all words are common stop words
        const stopWords = ['yang', 'dan', 'atau', 'dengan', 'untuk', 'dari', 'dalam', 'pada', 'ke', 'di', 'oleh', 'sebagai', 'adalah', 'merupakan', 'the', 'and', 'or', 'of', 'to', 'in', 'for', 'with', 'by', 'as', 'is', 'are'];
        const meaningfulWords = ngramWords.filter(word => !stopWords.includes(word) && word.length > 2);
    ...
    for (let i = 0; i <= words.length - n; i++) {
        ...
        if (isMeaningfulNGram(ngramWords)) {
...
function createFingerprint(text, windowSize = 50) {
    ...
    // Helper function to check if a window has meaningful content
    function hasSignificantContent(windowText) {
        const cleanText = windowText.toLowerCase().replace(/[^\w\s]/g, ' ');
        const wordsInWindow = cleanText.split(/\s+/).filter(word => word.length > 0);
        
        // Count meaningful words (not common words, numbers, or single characters)
        const meaningfulWords = wordsInWindow.filter(word => {
            ...
            // Skip common Indonesian/English stop words
            const stopWords = ['yang', 'dan', 'atau', 'dengan', 'untuk', 'dari', 'dalam', 'pada', 'ke', 'di', 'oleh', 'sebagai', 'adalah', 'merupakan', 'akan', 'dapat', 'harus', 'perlu', 'bisa', 'ada', 'tidak', 'juga', 'telah', 'sudah', 'masih', 'lebih', 'sangat', 'cukup', 'agak', 'ini', 'itu', 'tersebut', 'the', 'and', 'or', 'of', 'to', 'in', 'for', 'with', 'by', 'as', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'this', 'that', 'these', 'those'];
            
            return !stopWords.includes(word);
        });
        
        // Window should have at least 30% meaningful words and at least 3 meaningful words
        const meaningfulRatio = meaningfulWords.length / wordsInWindow.length;
        return meaningfulWords.length >= 3 && meaningfulRatio >= 0.3;
    }
    
    for (let i = 0; i <= words.length - windowSize; i++) {
        const window = words.slice(i, i + windowSize).join(' ');
        
        // Only create fingerprint if window has significant content
        if (hasSignificantContent(window)) {
            const hash = crypto.createHash('md5').update(window.toLowerCase()).digest('hex');
            fingerprints.push({
                hash: hash,
                position: i,
                content: window
...
function generateAIThinking(targetText, sourceText, similarity) {
    ...
    // Helper function to check if text is insignificant (should be ignored for plagiarism)
    function isInsignificantText(text) {
        const cleanText = text.trim().toLowerCase();
        
        // Ignore very short text (less than 3 characters)
        if (cleanText.length < 3) return true;
        
        // Ignore numbering patterns
        if (/^[\d\.\)\(\-\*\+\s]+$/.test(cleanText)) return true;
        
        // Ignore single numbers or letters with punctuation
        if (/^[a-z0-9][\.\)\(\-\s]*$/.test(cleanText)) return true;
        
        // Ignore common formatting patterns
        const insignificantPatterns = [
            ...
            /^[\d\s\.\-\(\)\[\]\/\\]+$/, // combinations of numbers and punctuation
            /^(dan|atau|serta|dengan|untuk|dari|ke|di|pada|dalam|oleh|sebagai|adalah|merupakan|yaitu|yakni|ialah)[\s\.]*/i, // common Indonesian conjunctions/prepositions
            /^(the|and|or|of|to|in|for|with|by|as|is|are|was|were|be|been|have|has|had|do|does|did|will|would|could|should|may|might)[\s\.]*/i // common English words
        ];
        
        return insignificantPatterns.some(pattern => pattern.test(cleanText));
    }
    
    // Helper function to check if sentence has meaningful content
    function hasMeaningfulContent(sentence) {
        ...
        // Check if it's mostly common words (should have some specific content)
        const commonWords = ['yang', 'dan', 'atau', 'dengan', 'untuk', 'dari', 'dalam', 'pada', 'ke', 'di', 'oleh', 'sebagai', 'adalah', 'merupakan', 'akan', 'dapat', 'harus', 'perlu', 'bisa', 'ada', 'tidak', 'juga', 'telah', 'sudah', 'masih', 'lebih', 'sangat', 'cukup', 'agak', 'the', 'and', 'or', 'of', 'to', 'in', 'for', 'with', 'by', 'as', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should'];
        const specificWords = words.filter(word => !commonWords.includes(word) && word.length > 2);
        
        // Must have at least 2 specific/meaningful words
    ...
    targetSentences.forEach((targetSentence, targetIndex) => {
        ...
        // Skip if this sentence is insignificant or has no meaningful content
        if (isInsignificantText(targetSentence) || !hasMeaningfulContent(targetSentence)) {
            return;
        }
        ...
        sourceSentences.forEach((sourceSentence, sourceIndex) => {
            ...
            // Skip if source sentence is also insignificant
            if (isInsignificantText(sourceSentence) || !hasMeaningfulContent(sourceSentence)) {
                return;
            }
...
// Enhanced plagiarism detection with AI thinking and Advanced Engine
function detectPlagiarism(targetText, sourceText, sourceDocument) {
    ...
    if (preprocessedTarget.length < 50 || preprocessedSource.length < 50) {
        return {
            overallSimilarity: 0,
            cosineSimilarity: 0,
            ngramSimilarity: 0,
            ...
            aiThinking: {
                ...
                nearMatches: [],
                suggestions: [],
                analysis: 'Teks terlalu pendek untuk analisis plagiarisme yang akurat.',
                severity: 'low',
                totalIssues: 0
            },
    ...
    // Use Advanced Engine for better detection
    const advancedResult = advancedEngine.detectAdvancedPlagiarism(targetText, sourceText, sourceDocument);
    
    // Fallback to original algorithm for comparison
    const originalResult = detectPlagiarismOriginal(targetText, sourceText, sourceDocument);
    
    // Combine results - use advanced engine as primary, original as fallback
    const combinedResult = {
        overallSimilarity: advancedResult.overallSimilarity,
        cosineSimilarity: originalResult.cosineSimilarity,
        ngramSimilarity: advancedResult.detailedAnalysis.ngramSimilarity,
        fingerprintSimilarity: originalResult.fingerprintSimilarity,
        
        // Advanced features
        detailedAnalysis: advancedResult.detailedAnalysis,
        entitiesDetected: advancedResult.entitiesDetected,
        ...
        matchingPhrases: originalResult.matchingPhrases || [],
        aiThinking: originalResult.aiThinking,
        sourceDocument: sourceDocument,
        isAdvanced: true
    };

    // Enhanced AI thinking with entity awareness
    if (combinedResult.aiThinking) {
        combinedResult.aiThinking.entityAnalysis = advancedResult.advancedInsights.entityAnalysis;
        combinedResult.aiThinking.semanticAssessment = advancedResult.advancedInsights.semanticAssessment;
        combinedResult.aiThinking.confidence = advancedResult.advancedInsights.confidence;
    }
    
    return combinedResult;
}

// Original plagiarism detection function (kept for fallback)
function detectPlagiarismOriginal(targetText, sourceText, sourceDocument) {
    const preprocessedTarget = preprocessText(targetText);
    const preprocessedSource = preprocessText(sourceText);
    ...
    // 3. Fingerprint Matching with adaptive window size (bobot: 25%)
    const targetWords = preprocessedTarget.split(' ').length;
    const sourceWords = preprocessedSource.split(' ').length;
    
    // Use larger window size to avoid matching insignificant phrases
    let windowSize = Math.min(Math.max(10, Math.floor(Math.min(targetWords, sourceWords) * 0.05)), 30);
    
    const targetFingerprints = createFingerprint(preprocessedTarget, windowSize);
    const sourceFingerprints = createFingerprint(preprocessedSource, windowSize);
    ...
    try {
        ...
        if (req.file && fs.existsSync(req.file.path)) {
            try {
                ...
                console.log(`🗑️ Cleaned up failed upload: ${req.file.originalname}`);
            } catch (unlinkError) {
        ...
        if (error.message.includes('PDF extraction failed')) {
            errorMessage = `PDF processing failed: ${error.message}. Try converting to TXT or DOCX format.`;
        } else if (error.message.includes('tidak dapat diekstrak')) {
            errorMessage = `File extraction failed: The file may be corrupted, password-protected, or in an unsupported format.`;
        }
        
        res.status(500).json({ 
            error: errorMessage,
            fileProcessed: req.file ? req.file.originalname : 'Unknown',
            suggestion: 'Try using TXT, DOCX, or a different PDF file'
        });
    }
});
...
app.post('/check-plagiarism', upload.single('targetFile'), async (req, res) => {
    try {
        ...
        if (req.file && fs.existsSync(req.file.path)) {
            try {
                fs.unlinkSync(req.file.path);
                console.log(`🗑️ Cleaned up failed check: ${req.file.originalname}`);
            } catch (unlinkError) {
                console.error('Error cleaning up file:', unlinkError.message);
            }
        }
        ...
        if (error.message.includes('PDF extraction failed')) {
            errorMessage = `PDF processing failed: Unable to extract text from the PDF file.`;
            suggestion = 'Try converting your PDF to TXT or DOCX format, or use a different PDF file.';
        } else if (error.message.includes('tidak dapat diekstrak')) {
            errorMessage = `File extraction failed: The file may be corrupted or password-protected.`;
            suggestion = 'Please check your file format and try again with a different file.';
        } else if (error.message.includes('Belum ada dokumen referensi')) {
            errorMessage = error.message;
            suggestion = 'Please upload reference documents first before checking for plagiarism.';
        }
        
        res.status(500).json({ 
            error: errorMessage,
            suggestion: suggestion,
            fileProcessed: req.file ? req.file.originalname : 'Text input',
