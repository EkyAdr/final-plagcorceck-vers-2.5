// components/history-analytics.js - Complete Enhanced History & Analytics Component
...
const HistoryAnalyticsComponent = {
    ...
    generateHTML: function() {
        return `
            <div class="history-analytics-wrapper">
                ...
                <!-- Statistics Overview -->
                <div id="history-stats" class="stats-grid">
                    <div class="stat-card">
    ...
    // Enhanced showHistoryDetails with detailed plagiarism breakdown
        ...
        try {
            ...
            if (data.success) {
                ...
                if (modal && modalTitle && modalBody) {
                    modalTitle.innerHTML = `📊 Detail Plagiarisme: ${entry.fileName}`;
                    modalBody.innerHTML = this.generateDetailedHistoryModalHTML(entry);
    ...
    // Generate detailed modal HTML with plagiarism breakdown
    generateDetailedHistoryModalHTML: function(entry) {
        const statusColor = Utils.getStatusColor(entry.status);
        
        return `
            <div class="detailed-history-modal">
                <!-- Overview Section -->
                <div class="history-overview-section">
                    <div class="overview-header">
                        <div class="file-info">
                            <div class="file-icon">${Utils.getFileIcon(entry.fileName)}</div>
                            <div class="file-details">
                                <h3>${entry.fileName}</h3>
                                <p class="file-meta">
                                    📅 ${new Date(entry.checkDate).toLocaleString('id-ID')} • 
                                    📝 ${(entry.textLength / 1000).toFixed(1)}K karakter • 
                                    📚 ${entry.totalDocumentsChecked} dokumen referensi
                                </p>
                            </div>
                        </div>
                        <div class="status-indicator">
                            ...
                            <div class="similarity-score" style="color: ${statusColor};">
                                ${entry.maxSimilarity}%
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="quick-stats">
                        <div class="stat-item">
                            <div class="stat-value">${entry.maxSimilarity}%</div>
                            <div class="stat-label">Max Similarity</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">${entry.summary?.avgCosine || 0}%</div>
                            <div class="stat-label">Avg Cosine</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">${entry.summary?.avgNgram || 0}%</div>
                            <div class="stat-label">Avg N-gram</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">${entry.summary?.avgFingerprint || 0}%</div>
                            <div class="stat-label">Avg Fingerprint</div>
                        </div>
                    </div>
                </div>

                <!-- Detailed Results Per Reference Document -->
                <div class="reference-breakdown-section">
                    <h4>📋 Breakdown per Dokumen Referensi</h4>
                    <p class="section-description">Detail tingkat kesamaan dengan setiap dokumen referensi</p>
                    
                    <div class="reference-results">
                        ${entry.detailedResults.map((result, index) => `
                            <div class="reference-result-item ${result.overallSimilarity >= 50 ? 'high-risk' : result.overallSimilarity >= 25 ? 'medium-risk' : 'low-risk'}">
                                <div class="reference-header" onclick="HistoryAnalyticsComponent.toggleReferenceDetail(${index})">
                                    <div class="reference-info">
                                        <div class="reference-name">
                                            ${Utils.getFileIcon(result.filename)} 
                                            <strong>${result.filename}</strong>
                                            <span class="file-type-tag">${result.fileType?.toUpperCase() || 'TXT'}</span>
                                        </div>
                                        <div class="similarity-main">
                                            <span class="similarity-percentage" style="color: ${Utils.getSimilarityColor(result.overallSimilarity)};">
                                                ${result.overallSimilarity}%
                                            </span>
                                            <span class="similarity-level">
                                                ${result.overallSimilarity >= 80 ? '🔴 Sangat Tinggi' : 
                                                  result.overallSimilarity >= 50 ? '🟠 Tinggi' : 
                                                  result.overallSimilarity >= 25 ? '🟡 Sedang' : '🟢 Rendah'}
                                            </span>
                                        </div>
                                    </div>
                                    <button class="expand-btn" title="Lihat Detail">
                                        <span id="expand-icon-${index}">⬇️</span>
                                    </button>
                                </div>
                                ...
                                <div class="similarity-progress">
                                    <div class="progress-bar">
                                        <div class="progress-fill" style="width: ${result.overallSimilarity}%; background-color: ${Utils.getSimilarityColor(result.overallSimilarity)};"></div>
                                    </div>
                                ...
                                <div class="reference-detail" id="reference-detail-${index}" style="display: none;">
                                    <div class="algorithm-breakdown">
                                        <h5>🔍 Breakdown Algoritma:</h5>
                                        <div class="algorithm-grid">
                                            <div class="algorithm-item">
                                                <div class="algorithm-name">Cosine Similarity</div>
                                                <div class="algorithm-value">${result.cosineSimilarity}%</div>
                                                <div class="algorithm-desc">Kesamaan berdasarkan frekuensi kata</div>
                                            </div>
                                            <div class="algorithm-item">
                                                <div class="algorithm-name">N-gram Analysis</div>
                                                <div class="algorithm-value">${result.ngramSimilarity}%</div>
                                                <div class="algorithm-desc">Kesamaan urutan kata (2-3 gram)</div>
                                            </div>
                                            <div class="algorithm-item">
                                                <div class="algorithm-name">Fingerprint Match</div>
                                                <div class="algorithm-value">${result.fingerprintSimilarity}%</div>
                                                <div class="algorithm-desc">Kesamaan pola kalimat unik</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    ${result.matchingPhrases && result.matchingPhrases.length > 0 ? `
                                        <div class="matching-phrases-section">
                                            <h5>🎯 Frasa yang Cocok:</h5>
                                            <div class="phrases-container">
                                                ${result.matchingPhrases.slice(0, 3).map(phrase => `
                                                    <div class="phrase-match">
                                                        <div class="phrase-icon">📝</div>
                                                        <div class="phrase-content">
                                                            <div class="phrase-text">"${phrase.substring(0, 100)}${phrase.length > 100 ? '...' : ''}"</div>
                                                            <div class="phrase-meta">Kesamaan tinggi dengan dokumen referensi</div>
                                                        </div>
                                                    </div>
                                                `).join('')}
                                                ${result.matchingPhrases.length > 3 ? `
                                                    <div class="more-phrases">
                                                        <small>+ ${result.matchingPhrases.length - 3} frasa lainnya dengan tingkat kesamaan tinggi</small>
                                                    </div>
                                                ` : ''}
                                            </div>
                                        </div>
                                    ...
                                    <!-- AI-Generated Specific Recommendations for this reference -->
                                    <div class="specific-recommendations">
                                        <h5>🤖 Rekomendasi Spesifik untuk Dokumen Ini:</h5>
                                        ${this.generateSpecificRecommendations(result)}
                                    </div>
                                </div>
                            </div>
                        `).join('')}
                ...
                <!-- AI Thinking Analysis Section -->
                ${this.generateAIThinkingSection(entry)}

                <!-- Overall AI Recommendations -->
                ${entry.aiRecommendations && entry.aiRecommendations.length > 0 ? `
                    <div class="ai-recommendations-section">
                        <h4>🎯 Rekomendasi Perbaikan Keseluruhan</h4>
                        <div class="recommendations-grid">
                            ${entry.aiRecommendations.map((rec, index) => `
                ...
                <div class="modal-actions">
                    <button class="btn btn-primary" onclick="ModalsComponent.showDownloadModal('${entry.id}')">
                        📥 Download Report Lengkap
                    </button>
                    ${entry.aiRecommendations[0]?.aiThinking ? `
                        <button class="btn btn-info" onclick="AIThinkingComponent.showModal('${entry.id}', ${JSON.stringify(entry.aiRecommendations[0].aiThinking).replace(/"/g, '&quot;')})">
                            🤖 AI Thinking Detail
                        </button>
                    ` : ''}
                    <button class="btn btn-warning" onclick="HistoryAnalyticsComponent.copyRecommendationsToClipboard('${entry.id}')">
            ...
            .reference-breakdown-section {
                margin: 25px 0;
            }

            .reference-breakdown-section h4 {
                color: #2c3e50;
                margin-bottom: 8px;
                display: flex;
                align-items: center;
            ...
            .reference-result-item.high-risk {
                border-left: 4px solid #dc3545;
                background: rgba(220, 53, 69, 0.02);
            }

            .reference-result-item.medium-risk {
                border-left: 4px solid #ffc107;
                background: rgba(255, 193, 7, 0.02);
            }

            .reference-result-item.low-risk {
            ...
            .recommendation-card.priority-wajib {
                border-left: 4px solid #dc3545;
            }

            .recommendation-card.priority-sedang {
                border-left: 4px solid #ffc107;
            }

            .recommendation-card.priority-rendah {
                border-left: 4px solid #28a745;
            }
    ...
    // Helper function to generate specific recommendations for each reference document
    generateSpecificRecommendations: function(result) {
        const recommendations = [];
        
        if (result.overallSimilarity >= 80) {
            recommendations.push({
                title: '🚨 Prioritas Utama - Rewrite Komprehensif',
                description: `Dokumen ini menunjukkan kesamaan ${result.overallSimilarity}% yang sangat tinggi. Diperlukan penulisan ulang komprehensif untuk bagian yang mengacu pada "${result.filename}".`
            });
        } else if (result.overallSimilarity >= 50) {
            recommendations.push({
                title: '⚠️ Perbaikan Diperlukan - Enhanced Paraphrasing',
                description: `Tingkat kesamaan ${result.overallSimilarity}% dengan "${result.filename}" memerlukan teknik parafrase yang lebih baik. Fokus pada perubahan struktur kalimat dan penggunaan sinonim.`
            });
        } else if (result.overallSimilarity >= 25) {
            recommendations.push({
                title: '💡 Optimasi Minor - Citation & Refinement',
                description: `Kesamaan ${result.overallSimilarity}% dengan "${result.filename}" masih dalam batas wajar, namun dapat ditingkatkan dengan sitasi yang tepat dan refinement minor.`
            });
        } else {
            recommendations.push({
                title: '✅ Kualitas Baik - Maintenance Mode',
                description: `Tingkat orisinalitas terhadap "${result.filename}" sudah sangat baik (${result.overallSimilarity}%). Pertahankan standar penulisan ini.`
            });
        }

        // Add specific algorithmic insights
        if (result.cosineSimilarity > result.ngramSimilarity + 20) {
            recommendations.push({
                title: '🔍 Insight: Kesamaan Vocabulari Tinggi',
                description: 'Cosine similarity tinggi menunjukkan penggunaan kata-kata yang sama. Cobalah menggunakan sinonim dan variasi terminologi.'
            });
        }

        if (result.ngramSimilarity > result.cosineSimilarity + 20) {
            recommendations.push({
                title: '🔍 Insight: Pola Kalimat Mirip',
                description: 'N-gram similarity tinggi menunjukkan struktur kalimat yang mirip. Variasikan pola kalimat dan urutan ide.'
            });
        }

        if (result.fingerprintSimilarity > 30) {
            recommendations.push({
                title: '🔍 Insight: Kemungkinan Copy-Paste',
                description: 'Fingerprint match tinggi mengindikasikan kemungkinan copy-paste. Periksa bagian-bagian yang mungkin disalin langsung.'
            });
        }

        return recommendations.map(rec => `
            <div class="specific-rec-item">
                <div class="specific-rec-title">${rec.title}</div>
                <div class="specific-rec-desc">${rec.description}</div>
            </div>
        `).join('');
    },

    // Generate AI Thinking section for the modal
    generateAIThinkingSection: function(entry) {
        const aiThinking = entry.aiRecommendations?.[0]?.aiThinking;
        ...
        return `
            <div class="ai-thinking-section">
                <h4>🤖 AI Thinking Analysis Summary</h4>
                <div class="ai-thinking-summary">
                    <div class="thinking-overview">
                        <div class="thinking-status ${aiThinking.severity}">
                            <span class="status-icon">
                                ${aiThinking.severity === 'high' ? '🚨' : aiThinking.severity === 'medium' ? '⚠️' : '✅'}
                            </span>
                            <span class="status-text">${aiThinking.analysis}</span>
                        </div>
                    </div>

                    <div class="thinking-stats">
                        <div class="thinking-stat">
                            <div class="stat-number">${aiThinking.totalIssues}</div>
                            <div class="stat-desc">Total Issues</div>
                        </div>
                        <div class="thinking-stat">
                            <div class="stat-number">${aiThinking.exactMatches.length}</div>
                            <div class="stat-desc">Exact Matches</div>
                        </div>
                        <div class="thinking-stat">
                            <div class="stat-number">${aiThinking.nearMatches.length}</div>
                            <div class="stat-desc">Near Matches</div>
                        </div>
                        <div class="thinking-stat">
                            <div class="stat-number">${aiThinking.suggestions.length}</div>
                            <div class="stat-desc">AI Suggestions</div>
                        </div>
                    </div>

                    ${aiThinking.exactMatches.length > 0 ? `
                        <div class="exact-matches-preview">
                            <h5>🚨 Sample Exact Matches:</h5>
                            ${aiThinking.exactMatches.slice(0, 2).map(match => `
                                <div class="match-preview">
                                    <div class="match-similarity">${match.similarity}%</div>
                                    <div class="match-text">"${match.original.substring(0, 80)}${match.original.length > 80 ? '...' : ''}"</div>
                                </div>
                            `).join('')}
                            ${aiThinking.exactMatches.length > 2 ? `<small>+${aiThinking.exactMatches.length - 2} more exact matches</small>` : ''}
                        </div>
                    ` : ''}

                    <div class="ai-thinking-actions">
                        <button class="btn btn-info" onclick="AIThinkingComponent.showModal('${entry.id}', ${JSON.stringify(aiThinking).replace(/"/g, '&quot;')})">
                            🤖 View Full AI Thinking Analysis
                        </button>
                    </div>
                </div>
            </div>
    ...
    // Toggle reference detail display
    toggleReferenceDetail: function(index) {
        const detail = document.getElementById(`reference-detail-${index}`);
        const icon = document.getElementById(`expand-icon-${index}`);
        
        if (detail && icon) {
            if (detail.style.display === 'none' || detail.style.display === '') {
                detail.style.display = 'block';
                Utils.animate.slideDown(detail, 300);
                icon.textContent = '⬆️';
            } else {
                Utils.animate.fadeOut(detail, 200);
                setTimeout(() => {
    ...
    async copyRecommendationsToClipboard(historyId) {
        try {
            const response = await fetch(`/check-history/${historyId}`);
            const data = await response.json();
            ...
            if (data.success) {
                const entry = data.historyEntry;
                let text = `📊 REKOMENDASI PLAGIARISME - ${entry.fileName}\n`;
                text += `==============================================\n\n`;
                text += `📅 Tanggal: ${new Date(entry.checkDate).toLocaleString('id-ID')}\n`;
                text += `📊 Similarity: ${entry.maxSimilarity}%\n`;
                text += `🏷️ Status: ${entry.status}\n\n`;

                if (entry.aiRecommendations && entry.aiRecommendations.length > 0) {
                    text += `🎯 REKOMENDASI AI:\n`;
                    text += `================\n\n`;
                    
                    entry.aiRecommendations.forEach((rec, index) => {
                ...
                if (entry.detailedResults && entry.detailedResults.length > 0) {
                    text += `📋 BREAKDOWN PER DOKUMEN:\n`;
                    text += `=========================\n\n`;
                    
                    entry.detailedResults.forEach((result, index) => {
                        text += `${index + 1}. ${result.filename} - ${result.overallSimilarity}%\n`;
                        text += `   • Cosine: ${result.cosineSimilarity}%\n`;
                        text += `   • N-gram: ${result.ngramSimilarity}%\n`;
                        text += `   • Fingerprint: ${result.fingerprintSimilarity}%\n\n`;
                    });
                }

                text += `\n---\n`;
                text += `Generated by Advanced Plagiarism Checker v2.1\n`;
                text += `${new Date().toLocaleString('id-ID')}\n`;

                await Utils.copyToClipboard(text);
                Utils.showAlert('📋 Rekomendasi berhasil disalin ke clipboard!', 'success');
            }
        } catch (error) {
            console.error('Error copying recommendations:', error);
            Utils.showAlert('Error copying recommendations: ' + error.message, 'error');
        }
    },
            ...
            const quickViewHtml = `
                <div class="quick-view-tooltip">
                    ...
                    <div class="quick-view-body">
                        ...
                        <p>AI Recs: ${entry.recommendationCount || 0}</p>
                    </div>
                </div>
            `;
            
            Utils.showAlert(quickViewHtml, 'info', 4000);
        }
    },
    ...
    deepAnalysisSelected: function() {
        if (this.state.selectedIds.length === 0) {
            Utils.showAlert('Pilih minimal satu history entry untuk deep analysis', 'warning');
            return;
        }
        this.bulkAnalysis();
    },
    ...
    exportForTesting: function() {
        return {
            ...
            generateDetailedHistoryModalHTML: this.generateDetailedHistoryModalHTML.bind(this)
        };
    },
